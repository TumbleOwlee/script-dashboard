...
SYSTEMD_CONFIG = {
    "none": [
        {"name": "InitialState", "options": ["enabled", "started"]},
    ],
    "Unit": [
        {
            "name": "Description",
        ...
        {"name": "Before", "placeholder": "Start this unit before specified units."},
        {"name": "After", "placeholder": "Start this unit after specified units."},
        {
            "name": "OnFailure",
    ...
    "Service": [
        ...
        {"name": "ExitType", "options": ["main", "cgroup"]},
        {"name": "RemainAfterExit", "options": ["no", "yes"]},
        {
            "name": "Restart",
            "options": [
                "no",
        ...
        {"name": "NotifyAccess", "options": ["none", "main", "exec", "all"]},
    ],
}
    ...
    if os.path.exists(script_dir):
        with open(os.path.join(script_dir, ".env/config.yml"), "r") as f:
            ...
            if config["scheduling"] == "systemd":
                service_file = f"{username}_{id}_generated.service"
                returncode, output = SYSTEM_SHELL.check_output(
                    ["systemctl", "start" if start else "stop", service_file.lower()]
                )
                if returncode == 0:
                    return {"id": id, "success": True}
                else:
            ...
            try:
                ...
                if output != "active":
                    shell = Shell(
                        ...
                        log_dir=os.path.join(working_dir, ".env"),
                    )
...
def upload(id):
    ...
    if (
        config["oldName"] is not None
        and id != config["oldName"]
    ...
    executable_path = (
        os.path.abspath(os.path.join(workspace, config["name"], config["executable"]))
    ...
    if "systemd" in config:
        working_dir = os.path.abspath(
            os.path.join(
                workspace,
                config["name"],
                config["systemd"]["Service"]["workingdirectory"],
            )
        )
        ...
        shell.run(
            [
                [
                    "chown",
                    "-R",
                    f"{config['username']}:{config['username']}",
                    new_script_dir,
                ],
                    ...
                    if value["name"] == "WorkingDirectory":
                        working_dir = os.path.abspath(
                            os.path.join(
                                workspace,
                                config["name"],
                                config["systemd"][entry]["workingdirectory"],
                            )
                        )
        ...
        ret = shell.run(
            [
                [
                    ...
                    config["username"],
                    "/usr/bin/bash",
                    os.path.join(env_dir, "install.sh"),
                ]
            ]
        )
        if ret != 0:
